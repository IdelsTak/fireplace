...
public class FirePlaceMain {
  ...
  private static void initUI(JFRBinder jfrBinder, List<Path> cliPaths) {
    ...
    if (Boolean.getBoolean("fireplace.swing.debug")) {
      if (Objects.equals(
          System.getProperty("fireplace.swing.debug.thread.violation.checker"), "IJ")) {
        ...
        () -> {
          ...
          var allocationFlameGraphPanel = new FlameGraphTab();
          {
            jfrBinder.bindEvents(
                ...
                allocationFlameGraphPanel::setStacktraceTreeModel);
          }
          var cpuFlameGraphPanel = new FlameGraphTab();
          {
            jfrBinder.bindEvents(
                JfrAnalyzer::stackTraceCPUFun, cpuFlameGraphPanel::setStacktraceTreeModel);
          }
          ...
          {
            jTabbedPane.addTab(
                SYSTEM_PROPERTIES, JScrollPaneWithButton.create(() -> new JScrollPane(sysProps)));
            jTabbedPane.addTab(
                NATIVE_LIBRARIES, JScrollPaneWithButton.create(() -> new JScrollPane(nativeLibs)));
          ...
          jfrBinder.setOnLoadActions(
              () -> hudPanel.setProgressVisible(true), () -> hudPanel.setProgressVisible(false));
  ...
  private static void setupLaF() {
    ...
    if (SystemInfo.isMacOS) {
      System.setProperty(
          ...
          "true"); // moves menu bar from JFrame window to top of screen
      System.setProperty(
          "apple.awt.application.name", "FirePlace"); // application name used in screen menu bar
    ...
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(
                () ->
                    ThemePreferencesHandler.getSharedInstance()
                        .enablePreferenceChangeReporting(false)));

    Runnable themeChanger =
        () -> {
          // System.out.println(">>>> theme preference changed = " +
          // ThemePreferencesHandler.getSharedInstance().getPreferredThemeStyle());
          switch (ThemePreferencesHandler.getSharedInstance()
    ...
    themeChanger.run();
    ThemePreferencesHandler.getSharedInstance()
        .addThemePreferenceChangeListener(e -> themeChanger.run());
  }
