 ...
 * dispatched.
 *
 * <p>The principle is to record the time at which we start processing an event, and have another
 ...
 *
 * <p>This is useful in determining what code is causing your Java application's GUI to be
 * unresponsive.
 *
 * @author Elliott Hughes <enh@jessies.org>
 *     <p>Advice, bug fixes, and test cases from Alexander Potochkin and Oleg Sukhodolsky.
 *     <p>Code extracted from the <a
 *     href="https://github.com/swingexplorer/swingexplorer/blob/7539add39b8b6741f59baab87a4034778c782113/swingexplorer-core/src/main/java/org/swingexplorer/edt_monitor/EDTDebugQueue.java#L221">swing-explorer's
 *     variation</a> and lightly modified by Brice Dutheil
 */
public final class EventDispatchThreadHangMonitor extends EventQueue {
  ...
  private static class DispatchInfo {
    ...
    private void examineHang() {
      ...
      if (isWaitingForNextEvent(currentStack)) {
        ...
        // As long as the modal dialog's event pump doesn't get stuck, it's okay for the outer pump
        // to be suspended.
        return;
      }
  ...
  /**
   * Overrides EventQueue.dispatchEvent to call our pre and post hooks either side of the system's
   * event dispatch code.
   */
  @Override
  protected void dispatchEvent(AWTEvent event) {
    try {
      ...
      if (monitorExceptions) {
        fireProblem(
            new Problem(
                "Exception caught in the Event Dispatch thread: " + ex.getMessage(),
                ex.getStackTrace()));
      } else {
        throw ex;
      }
