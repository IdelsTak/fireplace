...
final class JfrFilesDropHandler extends TransferHandler {
  ...
  public static void install(
      Consumer<List<Path>> pathsHandler, JComponent parent, DnDTarget target) {
    try {
      parent.setDropTarget(
          new DropTarget(
              parent,
              new DropTargetAdapter() {
                @Override
                public void dragEnter(DropTargetDragEvent dtde) {
                  var dataFlavorSupported =
  ...
  public boolean importData(TransferHandler.TransferSupport support) {
    ...
    var isJfrFiles = files.stream().allMatch(f -> f.isFile() && f.getName().endsWith(".jfr"));
    if (!isJfrFiles) {
      return false;
    }
