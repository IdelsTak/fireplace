...
public class JfrAnalyzer {
  ...
  static StacktraceTreeModel stackTraceAllocationFun(IItemCollection events) {
    ...
    var allocCollection =
        events.apply(
            ItemFilters.type(
                Set.of("jdk.ObjectAllocationInNewTLAB", "jdk.ObjectAllocationOutsideTLAB")));

    return new StacktraceTreeModel(
        allocCollection, methodFrameSeparator, false
        //                                       , JdkAttributes.ALLOCATION_SIZE
        );

    //        allocCollection.forEach(eventsCollection -> {
    //            var stackAccessor =
    // eventsCollection.getType().getAccessor(JfrAttributes.EVENT_STACKTRACE.getKey());
  ...
  static StacktraceTreeModel stackTraceCPUFun(IItemCollection events) {
    ...
    var allocCollection = events.apply(ItemFilters.type(Set.of("jdk.ExecutionSample")));

    var invertedStacks = false;
    return new StacktraceTreeModel(
        allocCollection, methodFrameSeparator, invertedStacks
                      ...
                      event -> {
                        ...
                        itemType
                            ...
                            .forEach(
                                (accessorKey) -> {
                                  System.out.println(
                                      accessorKey.getIdentifier()
                                          + "="
                                          + itemType.getAccessor(accessorKey).getMember(event));
                                });
                      });
            });
  }
  ...
  static String jvmSystemProperties(IItemCollection events) {
    ...
    events
        .apply(ItemFilters.type(Set.of("jdk.InitialSystemProperty")))
        .forEach(
            eventsCollection -> {
              var keyAccessor =
                  eventsCollection.getType().getAccessor(JdkAttributes.ENVIRONMENT_KEY.getKey());
              var valueAccessor =
                  eventsCollection.getType().getAccessor(JdkAttributes.ENVIRONMENT_VALUE.getKey());
              eventsCollection.stream()
                  .forEach(
                      event ->
                          stringBuilder
  ...
  static String nativeLibraries(IItemCollection events) {
    ...
    events
        .apply(ItemFilters.type(Set.of("jdk.NativeLibrary")))
        .forEach(
            eventsCollection -> {
              var nativeLibNameAccessor =
                  eventsCollection
                      .getType()
                      .getAccessor(JdkAttributes.NATIVE_LIBRARY_NAME.getKey());
              eventsCollection.stream()
                  .forEach(
                      event ->
                          stringBuilder
