 ...
 *
 * <p>Note this class a some field that are public and non final; this allows to quickly toy with
 * this tool, use with caution, or not at all.
 *
...
public class FlameGraphPainter<T> {
  ...
  public int computeFlameGraphMinimapHeight(int thumbnailWidth) {
    ...
    //            if (thumbnailWidth * (frame.endX - frame.startX) < 0.1 /* thumbnail visibility
    // threshold */) {
    //                continue;
    //            }
    //
  ...
  public Dimension computeFlameGraphDimension(Graphics2D g2, Rectangle visibleRect, Insets insets) {
    ...
    if (this.visibleWidth != visibleWidth) {
      var preferredFlameGraphWidth =
          ((int) (zoomFactor * visibleWidth)) - insets.left - insets.right;

      var visibleDepth = 0;
      for (var frame : frames) {
        if ((int) (preferredFlameGraphWidth * (frame.endX - frame.startX))
            < frameWidthVisibilityThreshold) {
          continue;
        }
      ...
      //                               " new fgWidth=" + preferredFlameGraphWidth + " visibleWidth="
      // + visibleWidth + " visibleDepth=" + visibleDepth + " zoom=" + zoomFactor);
    }

    //        System.out.println("getFlameGraphDimension, fgWidth=" + flameGraphWidth + "
    // visibleWidth=" + visibleWidth + " visibleDepth=" + visibleDepth + " zoom=" + zoomFactor);
  ...
  private float getFrameBoxTextOffset(Graphics2D g2) {
    return getFrameBoxHeight(g2)
        - (g2.getFontMetrics().getDescent() / 2f)
        - textBorder
        - frameGapWidth;
  }
  ...
  private void paint(Graphics2D g2, Rectangle visibleRect, boolean minimapMode) {
...

    {
      ...
      rectOnCanvas.width =
          ((int) (flameGraphWidth * rootFrame.endX)) - rectOnCanvas.x - internalPadding;
      ...
      if (visibleRect.intersects(rectOnCanvas)) {
        paintRootFrameRectangle(
            g2d,
            rectOnCanvas,
            rootFrameToText.apply(rootFrame.actualNode),
            handleFocus(
                frameColorFunction.apply(rootFrame.actualNode),
    ...
    for (int i = 1; i < frames.size(); i++) {
      ...
      rectOnCanvas.width =
          ((int) (flameGraphWidth * frame.endX)) - rectOnCanvas.x - internalPadding;
      ...
      if (visibleRect.intersects(rectOnCanvas)) {
        paintNodeFrameRectangle(
            g2d,
            rectOnCanvas,
            frame.actualNode,
            handleFocus(
                frameColorFunction.apply(frame.actualNode),
                hoveredFrame == frame,
                false,
                selectedFrame != null
    ...
    if (!minimapMode && paintDetails) {
      ...
      g2d.fillRect(
          visibleRect.x + visibleRect.width - nowWidth - textBorder * 2,
  ...
  private void paintHoveredFrameBorder(
      Graphics2D g2, Rectangle visibleRect, int frameBoxHeight, Rectangle rect) {
    ...
    rect.width =
        ((int) (flameGraphWidth * hoveredFrame.endX)) - rect.x - internalPadding - frameBorderWidth;
  ...
  private void identifyDisplayScale(Graphics2D g2) {
    ...
    //                          (transform.getType() & AffineTransform.TYPE_MASK_SCALE) ==
    // AffineTransform.TYPE_UNIFORM_SCALE ? "HiDPI " : "",
    //                          scaleX = transform.getScaleX(),
    //                          scaleY = transform.getScaleY());
  }

  private Color handleFocus(
      Color bgColor, boolean hovered, boolean highlighted, boolean dimmed, boolean minimapMode) {
    if (minimapMode) {
      return bgColor;
    }
    if (dimmed) {
  ...
  private Rectangle paintFrameRectangle(
      Graphics2D g2, Rectangle frameRect, Color bgColor, Color frameGapColor, boolean minimapMode) {
    var borderWidth = minimapMode ? 0 : frameGapEnabled ? frameGapWidth : 0;
  ...
  public Optional<FrameBox<T>> getFrameAt(Graphics2D g2, Point point) {
    ...
    return frames.stream()
        .filter(
            node -> node.stackDepth == depth && node.startX <= xLocation && xLocation <= node.endX)
        .findFirst();
  }

  public void toggleSelectedFrameAt(Graphics2D g2, Point point) {
    getFrameAt(g2, point).ifPresent(frame -> selectedFrame = selectedFrame == frame ? null : frame);
  }
    ...
    return getFrameAt(g2, point)
        .map(
            frame -> {
              ...
              flameGraphWidth =
                  (int) Math.max(visibleWidth / Math.min(1, frameWidthX), visibleWidth);
              ...
              return new Point((int) (frame.startX * flameGraphWidth), Math.max(0, y));
            });
  }
  ...
  private void paintFrameText(
      T node, Graphics2D g2, double targetWidth, Consumer<String> textConsumer) {
  ...
  private static void paintFrameText(
      String text, Graphics2D g2, double targetWidth, Consumer<String> textConsumer) {
    var metrics = g2.getFontMetrics();
