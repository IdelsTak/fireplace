...
/**
 * Builds a JLayer over a JScrollPane that displays a button that go back on top of the scroll pane.
 */
public abstract class JScrollPaneWithButton {
  ...
  public static JLayer<JScrollPane> create(Supplier<JScrollPane> scrollPaneSupplier) {
    return new JLayer<>(scrollPaneSupplier.get(), new ScrollBackToTopLayerUI(15, 15));
  }
  ...
  private static class ScrollBackToTopLayerUI extends LayerUI<JScrollPane> {
    ...
    private final JButton button =
        new JButton(
            new UpArrowIcon(new Color(0xAA_3D_42_44, true), new Color(0xAA_38_9F_D6, true))) {
          @Override
          ...
          protected void paintComponent(Graphics g) {
            ...
            if (getModel().isArmed()) {
              ...
              // g2.setColor(Colors.darkMode ? getBackground().brighter() :
    ...
    protected void processMouseEvent(MouseEvent e, JLayer<? extends JScrollPane> layer) {
      ...
      if (id == MouseEvent.MOUSE_CLICKED) {
        if (buttonRect.contains(currentMousePoint)) {
          ...
          && r.y > 0
          && buttonRect.contains(currentMousePoint)) {
        e.consume();
      }
    }
