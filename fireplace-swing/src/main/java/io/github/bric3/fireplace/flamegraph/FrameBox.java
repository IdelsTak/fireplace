 ...
 *
 * <p>This wrapper is used to pre-process, the depth, the boundaries on the X axis (horizontal) as
 * ratios of that space, likely {@code [0.0, 1.0]}.
 *
 * <p>In order to pre-compute this list you can use {@link #flattenAndCalculateCoordinate(List,
 * Object, Function, ToDoubleFunction, double, double, int)} utility method.
 *
 * @param <T>
 * @see FlameGraph
 */
public class FrameBox<T> {
  ...
  public String toString() {
    return "FlameNode{" + "startX=" + startX + ", endX=" + endX + ", depth=" + stackDepth + '}';
  }
  ...
  public static <T> void flattenAndCalculateCoordinate(
    ...
    for (var node : children) {
      ...
      flattenAndCalculateCoordinate(
          accumulator, node, getChildren, nodeWeight, startX, endX, depth);
      startX = endX;
    }
  }
}
